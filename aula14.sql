...
CREATE OR REPLACE FUNCTION atualizar_qtde_projetos_delete()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_atualizar_qtde_projetos_delete
AFTER DELETE ON tbl_projetos
FOR EACH ROW
EXECUTE FUNCTION atualizar_qtde_projetos_delete();

-- (2) Crie uma Trigger que atualiza a quantidade de projetos da tbl_departamentos caso um novo projeto for adicionado.
CREATE OR REPLACE FUNCTION atualizar_qtde_projetos_insert()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE tbl_departamentos
    SET qtde_projetos = qtde_projetos + 1
...
CREATE TRIGGER trg_atualizar_qtde_projetos_insert
AFTER INSERT ON tbl_projetos
FOR EACH ROW
EXECUTE FUNCTION atualizar_qtde_projetos_insert();
...
CREATE TABLE tbl_empregado_historico_salario (
    ...
    cod_empregado INT NOT NULL REFERENCES tbl_empregados(cod_empregado),
    salario REAL NOT NULL,
    data TIMESTAMP DEFAULT now()
);

--  (3-B) Crie uma trigger que salva o salário atual de um empregado toda vez que seu salário for atualizado [dica: use a funcao now() para salvar a data]
...
BEGIN
    ...
    VALUES (NEW.cod_empregado, OLD.salario, now());
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_historico_salario
BEFORE UPDATE OF salario ON tbl_empregados
FOR EACH ROW
...
-- (4-B) Crie uma Trigger que toda vez que os dados do empregado forem atualizados, que essa data também seja atualizada.
CREATE OR REPLACE FUNCTION atualizar_data_empregado()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_atualizar_data_empregado
BEFORE UPDATE ON tbl_empregados
FOR EACH ROW
EXECUTE FUNCTION atualizar_data_empregado();
