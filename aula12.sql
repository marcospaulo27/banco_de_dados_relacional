...
CREATE OR REPLACE FUNCTION fc_soma(valor1 INT, valor2 INT) 
RETURNS INT AS $$
BEGIN
...
BEGIN
    FOR nome_empregado IN 
        SELECT nome FROM tbl_empregados 
        WHERE salario = (SELECT MAX(salario) FROM tbl_empregados)
    LOOP
...
CREATE OR REPLACE FUNCTION fc_salarios(p_cod_empregado INT) 
RETURNS TABLE(salario_atual REAL, salario_acrescido REAL, salario_reduzido REAL) AS $$
BEGIN
    RETURN QUERY 
    SELECT 
        salario::REAL, 
...
BEGIN
    RETURN QUERY 
    SELECT tbl_projetos.cod_projeto, tbl_projetos.nome 
    FROM tbl_projetos;
END;
$$ LANGUAGE plpgsql;
...
-- (6) Crie a função fc_proj_departmento que recebe o código do departamento e mostra quantos projetos o departamento possui.
